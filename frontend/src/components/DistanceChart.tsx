...
const defaultData: DistanceData[] = [
  { time: '00:00', distance: 750 },
  { time: '04:00', distance: 800 },
  { time: '08:00', distance: 850 },
  { time: '12:00', distance: 900 },
  { time: '16:00', distance: 880 },
  { time: '20:00', distance: 860 },
  { time: '24:00', distance: 870 },
];

export const DistanceChart = () => {
  const { theme } = useTheme();
  const { refreshTrigger } = useRefreshTimer();
  const [data, setData] = useState<DistanceData[]>(defaultData);
  ...
  const fetchDistanceData = async () => {
    ...
    try {
      ...
      const response = await apiClient.getDistanceReadings(1);
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold">Distance Tracker</h3>
        ...
        <LineChart data={data}>
          <defs>
            <linearGradient id="distanceGradient" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={isDark ? "hsl(var(--neon-cyan))" : "hsl(186 100% 37%)"} stopOpacity={0.8} />
              <stop offset="100%" stopColor={isDark ? "hsl(var(--neon-cyan))" : "hsl(186 100% 37%)"} stopOpacity={0.1} />
            </linearGradient>
          </defs>
          ...
          <Line
            type="monotone"
            dataKey="distance"
            stroke={isDark ? "hsl(var(--neon-cyan))" : "hsl(186 100% 37%)"}
            strokeWidth={3}
            dot={{ fill: isDark ? "hsl(var(--neon-cyan))" : "hsl(186 100% 37%)", r: 4 }}
            activeDot={{ r: 6 }}
            className={isDark ? "neon-glow-cyan" : ""}
          />
        </LineChart>
      </ResponsiveContainer>
    </div>
